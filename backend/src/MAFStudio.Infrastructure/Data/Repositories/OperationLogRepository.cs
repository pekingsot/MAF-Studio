...
public class OperationLogRepository : IOperationLogRepository
{
    ...
    public async Task<List<OperationLog>> GetByUserIdAsync(long userId, int limit = 100)
    {
        using var connection = _context.CreateConnection();
        const string sql = @"
    ...
    public async Task<OperationLog> CreateAsync(OperationLog log)
    {
        ...
        const string sql = @"
            INSERT INTO operation_logs (
                user_id, action, resource_type, resource_id, description, details, 
                ip_address, user_agent, request_path, request_method, status_code, duration_ms, error_message, created_at
            ) VALUES (
                @UserId, @Action, @ResourceType, @ResourceId, @Description, @Details,
